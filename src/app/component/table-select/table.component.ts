...
@Component({
  selector: 'tcx-table-select',
  imports: [CommonModule],
  templateUrl: './table.component.html',
  styleUrl: './table.component.css',
})
export class TableComponentSelect implements OnInit, OnChanges {
  ...
  @Input() permitirSelecao: boolean = false;
  @Input() chaveSelecao: string = 'id';
  @Input() podeSelecionar?: (item: any) => boolean;
  ...
  // SELECIONAR
  get itensSelecionaveis(): any[] {
    return (this.dados ?? []).filter((item) =>
      this.podeSelecionar ? this.podeSelecionar(item) : true,
    );
  }

  isSelecionado(item: any): boolean {
    return this.selecionados.has(item[this.chaveSelecao]);
  }

  selecionarItem(item: any, event: Event): void {
    event.stopPropagation();

    const checked = (event.target as HTMLInputElement).checked;
    const id = item[this.chaveSelecao];
  ...
  selecionarTodos(event: Event): void {
    ...
    if (checked) {
      this.itensSelecionaveis.forEach((item) =>
        this.selecionados.add(item[this.chaveSelecao]),
      );
    } else {
      this.itensSelecionaveis.forEach((item) =>
        this.selecionados.delete(item[this.chaveSelecao]),
      );
    }
  ...
  todosSelecionados(): boolean {
    return (
      this.itensSelecionaveis.length > 0 &&
      this.itensSelecionaveis.every((item) => this.isSelecionado(item))
    );
  }

  private emitirSelecionados(): void {
    const lista = (this.dados ?? []).filter((item) =>
      this.selecionados.has(item[this.chaveSelecao]),
    );

    this.selecionadosChange.emit(lista);
  }
